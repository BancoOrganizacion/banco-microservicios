...
export class FingerprintService {
  constructor(
    @InjectModel(DedoRegistrado.name)
    private dedoRegistradoModel: Model<DedoRegistrado>,
  ...
  async createFingerPattern(createFingerpatternDto: CreateFingerpatternDto) {
    ...
    if (createFingerpatternDto.pattern.length !== 5) {
      throw new BadRequestException(
        'El patrón debe contener exactamente 5 dedos',
      );
    }
    ...
    if (uniqueDedos.size !== 5) {
      throw new BadRequestException(
        'Los 5 dedos del patrón deben ser diferentes',
      );
    }
    ...
    const patternEntries = await Promise.all(
      createFingerpatternDto.pattern.map(async (fingerData, index) => {
        const matchingFinger = registeredFingers.find(
          (f) => f.dedo === fingerData.dedo,
        );
