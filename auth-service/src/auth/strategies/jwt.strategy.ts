...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(payload: any) {
    this.logger.debug(
      `Validando token para usuario ID: ${payload.id_usuario}, Rol ID: ${payload.id_rol}`,
    );
    ...
    try {
      ...
      if (!usuario) {
        this.logger.warn(
          `Usuario con ID ${payload.id_usuario} no encontrado durante la validación del token`,
        );
      ...
      if (usuario.rol._id.toString() !== payload.id_rol) {
        this.logger.warn(
          `Rol del usuario ${payload.id_usuario} no coincide con el rol en el token`,
        );
