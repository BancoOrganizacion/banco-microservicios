...
export class AuthController {
  ...
  async generateRegistrationCode(
    @Body() registerCodeDto: RegisterCodeDto,
    @Request() req,
  ) {
    try {
      ...
      const code = await this.authService.generateRegistrationCode(
        userId,
        registerCodeDto.tipo,
      );
      return { code };
    } catch (error) {
  ...
  async generateTokenAfterRegistration(data: {
    username: string;
    userId: string;
    rolId: string;
  }) {
    try {
      const token = await this.authService.generateTokenForNewUser(
        data.userId,
        data.rolId,
      );
      return { access_token: token };
      ...
      this.logger.error(
        `Error al generar token después del registro: ${error.message}`,
      );
  ...
  async getUserByCode(@Body() getUserByCodeDto: GetUserByCodeDto) {
    try {
      const userId = await this.authService.getUserIdFromCode(
        getUserByCodeDto.code,
      );
      ...
      this.logger.error(
        `Error al obtener usuario por código: ${error.message}`,
      );
      throw new HttpException(
